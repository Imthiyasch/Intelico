...
function AuthForm() {
  ...
  const handleGuestSignIn = async () => {
    ...
    if (!error) {
      ...
      alert("Failed to login as guest. Make sure Anonymous Sign-ins are enabled in Supabase.");
    }
  };
      ...
      <div className="relative w-full max-w-md animate-fade-in">
        ...
        <div className="bg-white rounded-3xl p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 text-center">
          ...
          <p className="text-center text-xs text-slate-400 mt-8">
            ...
            <Link href="#" className="text-slate-500 hover:text-slate-700 underline underline-offset-2">Terms</Link>{" "}
            &amp;{" "}
...
export default function AuthPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-50 flex items-center justify-center"><Loader2 className="w-8 h-8 text-blue-600 animate-spin" /></div>}>
      <AuthForm />
    </Suspense>
  );
}
